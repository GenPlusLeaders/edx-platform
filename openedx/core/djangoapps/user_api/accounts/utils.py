...
def is_staff_progress_tracking_disabled(user):
    tracking_disabled = not hasattr(user, 'gen_user')
    if hasattr(user, 'gen_user'):
        user_is_staff = user.gen_user.is_teacher
        tracking_disabled = user_is_staff and TEACHER_PROGRESS_TACKING_DISABLED_SWITCH.is_enabled()
