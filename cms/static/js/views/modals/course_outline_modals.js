...
define(['jquery', 'backbone', 'underscore', 'gettext', 'js/views/baseview',
  ...
  var CourseOutlineXBlockModal, SettingsXBlockModal, PublishXBlockModal, HighlightsXBlockModal,
    ...
    ThumbnailUploadEditor, LessonTypeEditor, LedTypeEditor, CompletionTrackingEditor, LMSLeftNavEditor;
  ...
  LMSLeftNavEditor = AbstractEditor.extend({
    templateName: 'left-nav-info',
    className: 'left-nav-info',

    afterRender: function () {
      AbstractEditor.prototype.afterRender.call(this);
      this.$('input[name=lms-nav-group-name-input]').val(this.model.get('genplus_chapter_group'));
      this.$('input[name=lms-nav-group-time-input]').val(this.model.get('genplus_chapter_group_time'));
    },
    currentValue: function () {
      return {
        genplus_chapter_group: this.$('input[name=lms-nav-group-name-input]').val(),
        genplus_chapter_group_time: this.$('input[name=lms-nav-group-time-input]').val()
      }
    },
    getRequestData: function () {
      if (this.hasChanges()) {
    ...
    hasChanges: function () {
      var currentState = this.currentValue();
      return (currentState.genplus_chapter_group != this.model.get('genplus_chapter_group') || currentState.genplus_chapter_group_time != this.model.get('genplus_chapter_group_time'));
    },
  });
    ...
    getEditModal: function (xblockInfo, options) {
      ...
      if (xblockInfo.isVertical()) {
        ...
        if (xblockInfo.isChapter()) {
          ...
          advancedTab.editors.push(LessonTypeEditor, CompletionTrackingEditor, LMSLeftNavEditor);
